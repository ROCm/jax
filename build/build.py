...
def add_global_arguments(parser: argparse.ArgumentParser):
    ...
    parser.add_argument(
        "--verbose", action="store_true", help="Produce verbose output for debugging."
    )
...
def add_artifact_subcommand_arguments(parser: argparse.ArgumentParser):
    ...
    cuda_group.add_argument(
        "--disable_nccl", action="store_true", help="Should NCCL be disabled?"
    )
    ...
    rocm_group.add_argument(
        "--rocm_version", type=str, default="60", help="ROCm version to use"
    )
    ...
    rocm_group.add_argument(
        "--rocm_path", type=str, default="", help="Path to the ROCm toolkit."
    )
...
async def main():
    ...
    if args.command == "requirements_update":
        ...
        if args.nightly_update:
            ...
            requirements_command.append(
                f"//build:requirements{ft_suffix}_nightly.update"
            )
        else:
            requirements_command.append(f"//build:requirements{ft_suffix}.update")

        result = await executor.run(
            requirements_command.get_command_as_string(),
            args.dry_run,
    ...
    if args.use_clang:
        ...
        logging.debug("Using GCC as the compiler, gcc path: %s", gcc_path)
    ...
    if args.target_cpu_features == "release":
        if arch in ["x86_64", "AMD64"]:
            ...
            wheel_build_command_base.append(
                "--config=avx_windows" if os_name == "windows" else "--config=avx_posix"
            )
    elif args.target_cpu_features == "native":
        if os_name == "windows":
            logger.warning(
                "--target_cpu_features=native is not supported on Windows;" " ignoring."
            )
        else:
    ...
    if "cuda" in args.wheels:
        ...
        if args.cuda_compute_capabilities:
            logging.debug(
                "Hermetic CUDA compute capabilities: %s", args.cuda_compute_capabilities
            )
            wheel_build_command_base.append(
    ...
    # .bazelrc or above.
    if args.bazel_options:
        logging.debug("Additional Bazel build options: %s", args.bazel_options)
        for option in args.bazel_options:
    ...
    with open(".jax_configure.bazelrc", "w") as f:
        jax_configure_options = utils.get_jax_configure_bazel_options(
            wheel_build_command_base.get_command_as_list(),
            args.use_new_wheel_build_rule,
        )
        if not jax_configure_options:
            logging.error(
                "Error retrieving the Bazel options to be written to .jax_configure.bazelrc, exiting."
            )
            sys.exit(1)
    ...
    if args.configure_only:
        ...
        for wheel in args.wheels.split(","):
            ...
            print("\n")
            logger.info("Building %s for %s %s...", wheel, os_name, arch)
            ...
            if not args.use_new_wheel_build_rule:
                ...
                if "cuda" in wheel:
                    ...
                    wheel_build_command.append(
                        f"--platform_version={cuda_major_version}"
                    )

                if "rocm" in wheel:
                    wheel_build_command.append("--enable-rocm=True")
                    wheel_build_command.append(
                        f"--platform_version={args.rocm_version}"
                    )
