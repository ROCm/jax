...
GetHostAndInitFuncNames(mlir::ModuleOp module_op) {
...
absl::StatusOr<CompiledKernel *> CachedCompileAndInit(CacheKey key,
...

  {
    ...
    if (ABSL_PREDICT_TRUE(it != cache.kernels.end()))
      return &it->second;
  }
...
absl::Status MosaicGpuExecute(gpuStream_t stream, ffi::RemainingArgs inputs,
  ...
  gpuContext_t ctx;
  if (gpuCtxGetCurrent(&ctx) != gpuSuccess) {
    fprintf(stderr, "Failed to get current GPU context\n");
    abort();
  }
  CacheKey key(hash, reinterpret_cast<uintptr_t>(ctx));
  TF_ASSIGN_OR_RETURN(auto compiled_kernel, CachedCompileAndInit(key, module));
  auto ctx_kernel_comm = compiled_kernel->GetHostLaunch();
  bool is_comm_used = std::get<2>(ctx_kernel_comm);
