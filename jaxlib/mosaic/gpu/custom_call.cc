...
GetPtxIsaVersion(const se::cuda::CompilationProvider &compilation_provider) {
...
absl::StatusOr<std::unique_ptr<CompiledKernel>> Compile(
  llvm::StringRef module_str, const se::GpuComputeCapability* absl_nullable gpu_cc) {
  ...
  const se::CudaComputeCapability* cc{}; // mustn't initialize to &cc_obj, UB
  if (gpu_cc == nullptr) {
  ...
  const se::RocmComputeCapability* cc{}; // mustn't initialize to &cc_obj, UB
  if (gpu_cc == nullptr) {
  ...
  auto passes = GetPassPipelineCUDA(module->getContext(), compilation_provider, *cc,
...
MosaicGpuCompile(const char *module, int num_module_bytes) {
