...
def mha_bwd_unified(dout, q, k, v, out, lse, dropout_p, softmax_scale,
    ...
    fn = _make_bwd_call(dq_shape, dk_shape, dv_shape, sd_shape, dbias_shape, q.dtype)
...
def _apply_1block_override(use_v3, sk, dq, deterministic, gfx):
    """gfx950 1-block override: only relevant when ASM v3 is active."""
    is_950_1block = (
        use_v3
        and gfx == "gfx950"
        and sk <= 256
...
def _flash_attn_backward(dout, q, k, v, out, lse,
    ...
    gfx = get_gfx()
    bf16_cvt = 0 if gfx == "gfx950" else 1

    use_v3 = _compute_v3_eligibility_bwd(
        dropout_p, hq, hk, dq, causal, wl, wr, bias, sq, gfx
    )
    bwd_det, bwd_atomic = _apply_1block_override(
        use_v3, sk, dq, deterministic, gfx
    )
...
def flash_attn_func(
    ...
    q_p, k_p, v_p, hd_q_og, hd_v_og = _pad_to_multiple_of_8(q, k, v)
...
def _flash_attn_varlen_bwd(max_seqlen_q, max_seqlen_k, dropout_p,
    ...
    hq = q_p.shape[1] if q_p.ndim == 3 else q_p.shape[2]
    dq = q_p.shape[2] if q_p.ndim == 3 else q_p.shape[3]
    ...
    use_v3 = _compute_v3_eligibility_bwd(
        ...
        None, max_seqlen_k, gfx
    )
    bwd_det, bwd_atomic = _apply_1block_override(
        use_v3, res_max_sk, dq, res_det, gfx
    )
