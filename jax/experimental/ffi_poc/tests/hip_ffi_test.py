...
from jax_ffi_poc import hip_ffi  # pylint: disable=g-import-not-at-top
...
class CudaE2eTests(jtu.JaxTestCase):
  ...
  def setUp(self):
    ...
    from jax_ffi_example import hip_ffi  # pylint: disable=g-import-not-at-top

    self.foo = hip_ffi.foo
...
def test_bwd_random():
  print("test_bwd_random")
  shape = (2, 3)
  akey, bkey = jax.random.split(jax.random.key(0))
  a = jax.random.normal(key=akey, shape=shape, dtype=jnp.float32)
  b = jax.random.normal(key=bkey, shape=shape, dtype=jnp.float32)
  jtu.check_grads(hip_ffi.neuron, args=(a, b), order=1, modes=("rev",), atol=1e-1, rtol=1e-1 )
